<template>
  <div
    class="editor"
    :class="themeStore.isDarkmode ? editorDarkmodeClass : editorNormalClass"
    style="height: 100%"
    :style="{}"
    ref="editor"
  >
    <Editor
      :id="id"
      :class="inline ? 'ipac-inline-editor' : 'ipac-editor'"
      :init="{
        ...init,
        skin: false,
        content_css: '/tinymce/content.min.css',
        setup: editorSetup,
        init_instance_callback: editorCallback,
      }"
      :style="{
        ...style,
        border: inline
...
const props = defineProps<{
  ...
  claimEditor?: boolean;
  disabled?: boolean;
  doNotUseAutoCapitalize?: boolean;
}>();
  ...
  editor.ui.registry.addButton("settextindentzero", {
    ...
    onAction: function () {
      ...
      if (node.tagName === "P") {
        ...
        // 노드가 p가 아니면 p인 부모를 찾아서 서식 적용
        let parentElement = node.cloneNode(true).parentElement;
        let i = 0;
        while (
...
function editorSetup(editor: EditorType) {
  ...
  if (props.useAutocomplete) _tinyConfig.value(editor);
  if (!props.inline) {
...
// 에디터 데이터 셋팅
function editorCallback(editor: EditorType) {
  // data가 있으면, 셋팅해줌
  // console.log(props.data);
  if (props.id && props.data) tinymce.get(props.id).setContent(props.data);

  // init에 callback으로 지정해둔게 있으면, 실행함
  if (props.init.callback) {
    props.init.callback(editor);
  }
  // 유저세팅에서 다크모드 유무 불러와서 세팅하기
  if (themeStore.isDarkmode) editor.getBody().classList.add("inner-dark");
}
...
// 자동완성 type.
let autocompleteType = Util.getSettingValue(
  stores,
  "SETTING/EDITOR/AUTOCOMPLETE_FORMAT"
);

// 선행사 설정 배열 -> 여러개가 들어 있음. 구분자는 +
const editorAntecedentArray = Util.getSettingValue(
  stores,
  "SETTING/EDITOR/ANTECEDENT"
)?.split("+");
let editorAntecedent = "";
// 청구항 상기 오류 체크 여부.
let claimCheckAntecedent = Util.getSettingValue(
  stores,
  "SETTING/DRAFT/CLAIM_CHECK_ANTECEDENT_ERROR"
);
// 상세한 설명 상기 오류 체크 여부.
let descriptionCheckAntecedent = Util.getSettingValue(
  stores,
  "SETTING/DRAFT/DESCRIPTION_CHECK_ANTECEDENT_ERROR"
);
...
_tinyConfig.value = (editor: EditorType) => {
  ...
  editor.on("keydown", function (e) {
    ...
    // 패널이 떠 있지 않은 경우(일반적인 타이핑) 이벤트 핸들링
      ...
      // tab 키 눌렀을 때 간격 추가
      if (e.key == "Tab") {
        editorSelection = tinymce.get(props.id).selection;
          ...
          if (
            ...
            if (
              ...
              if (editorRange.endContainer.previousSibling === null) {
                ...
                if (currentOffset === 0) {
                  tinymce.get(props.id).insertContent("</p><p>");
                  tinymce.get(props.id).execCommand("ForwardDelete");
                  return;
                }
              }
            }
    ...
    if (
      ...
      editorRange = tinymce.get(props.id).selection.getRng();
      ...
      if (replaceTxt != "") {
        ...
        tinymce.get(props.id).insertContent(replaceTxt);
      }
    }
  });
...
watch(workspaceStore, () => {
  if (isPanelOn.value) panelSelected.value = 0;
});
...
const checkAutocomplete = (e: InputEvent) => {
  ...
  if (candidateElementList.length > 0 && candidateElementList[1].length > 0) {
    ...
    if (workspaceStore.caseMasterData.format_code_path == "FORMAT/USPTO") {
      isFirstWordInSentence = checkFirstWordInSentence(
        tinymce.get(props.id).selection.getRng()
      );
    }
    ...
    try {
      ...
      let tinymceHeader = tinymce
        .get(props.id)
        .editorContainer.getElementsByClassName("tox-editor-header")[0];
      ...
      if (rects?.item(0) !== null && rects?.item(0) !== undefined) {
        ...
        _top =
          ...
          tinymceHeaderHeight +
          parseInt(stores.ws.fontSize.substring(0, 2)) +
          10;
        // 인라인일 때 값 조정
        if (props.inline) {
          _left -= editorX - 5;
      ...
      if (windowHeight !== undefined && windowWidth !== undefined) {
        ...
        if (_isAutoBoxOverflowHeight) {
          autoBoxTop.value =
            _top -
            autoBoxHeight +
            (parseInt(workspaceStore.fontSize.substring(0, 2))
              ? parseInt(workspaceStore.fontSize.substring(0, 2)) - 15
              : 0) -
            parseInt(stores.ws.fontSize.substring(0, 2)) -
            10;
          _handleAutoBoxWidth();
...
const generateCandidateElementList = (keyword: string | undefined) => {
  ...
  if (
    workspaceStore.caseMasterData.format_code_path == "FORMAT/KIPO" ||
    workspaceStore.caseMasterData.format_code_path == "FORMAT/USPTO"
  ) {
    // 구성요소 리스트 돌면서, 각 아이템의 명칭 및 번호랑 keyword랑 비교해서, 포함되어 있으면 결과에 포함
    // console.log(workspaceStore.caseData.elements.list);
    workspaceStore.caseData.elements.list.forEach((item: ElementData) => {
      if (
  ...
  // FORMAT/JPO인 경우 keyword를 추출함. 나중에 CNIPA(중국) 추가되면 FORMAT/JPO처럼 분기 태워야할 수 있음.
    ...
    for (let i = (keywordArray as string[]).length - 1; i >= 0; i--) {
      ...
      workspaceStore.caseData.elements.list.forEach((item) => {
  ...
  // 도면부호 있는 구성요소와, 없는 구성요소와, 관용어로 나눔
  ...
  const tempList2 = searchResultList.filter(
    (element) =>
      Util.removePTag(element.number).trim().length === 0 &&
      element.itemType === "E"
  );
  const tempList3 = searchResultList.filter(
    (element) =>
      Util.removePTag(element.number).trim().length === 0 &&
      element.itemType !== "E"
  );
  ...
  tempList1.sort(function (a, b) {
    ...
    let a_num = 0;
    if (
      Util.toJPHalfWidth(Util.removePTag(a.number).toLowerCase()).match(
        /(\d+)/g
      )
    ) {
      a_num = Number(
    ...
    // a에서 숫자 제외한 나머지
    let a_text = Util.removePTag(a.number.replace(a_num + "", ""));
    a_text = Util.removePTag(
      a.number.replace(Util.toJPFullWidth(a_num + ""), "")
    );
    // b에서 숫자만 뽑음
    let b_num = 0;
    if (
      Util.toJPHalfWidth(
        Util.removePTag(b.number).toLowerCase().match(/(\d+)/g)
      )
    ) {
      b_num = Number(
        Util.toJPHalfWidth(Util.removePTag(b.number).toLowerCase()).match(
          /(\d+)/g
        )[0]
      );
    }
    // b에서 숫자 제외한 나머지
    let b_text = Util.removePTag(b.number.replace(b_num + "", ""));
    b_text = Util.removePTag(
      b.number.replace(Util.toJPFullWidth(b_num + ""), "")
    );

    // 숫자만으로 정렬(오름차순)이 되면 그대로 리턴
    if (a_num > b_num || a_num < b_num) return a_num - b_num;
    // 숫자만으로 정렬이 안되면 텍스트로 정렬(오름차순)해서 리턴
    else {
  ...
  // 도면부호가 없는 경우에는 명칭으로 정렬해 줌
  tempList2.sort(function (a, b) {
    let a_name = Util.removePTag(a.name);
    let b_name = Util.removePTag(b.name);
    if (a_name < b_name) return -1;
    else if (a_name > b_name) return 1;
    else return 0;
  });

  // 도면부호 없는거랑 있는거 + 관용어 합쳐 줌
  const returnResult = tempList1.concat(tempList2).concat(tempList3);
...
const changeToAutocompleteText = (item: ElementData) => {
  ...
  if (!antecedentError && props.claimEditor) {
    // console.log("청구항 에디터다");

    const editingParentArray = (
      document.getElementById("hidden-parent") as HTMLInputElement
    )?.value?.split(",");

    const editingClaimType = (
      document.getElementById("hidden-claim-type") as HTMLInputElement
    ).value;

    const editingClaimNumber = parseInt(
      (document.getElementById("hidden-claim-number") as HTMLInputElement)
        ?.value
    );

    // console.log(editingParentArray, editingClaimNumber, editingClaimType);

    // 청구항에서 작성하는 경우에는 아래 checkElementInParentClaims 결과까지 같이 비교해야 함. 둘다 true여야 선행사 에러 없는 것임
    // 독립항인 경우에는 pass
    if (editingClaimType === "D") {
      antecedentError = checkElementInParentClaims(
  ...
  // );
  // if (!antecedentError)
  //   tinymce.get(props.id).insertContent("<span style=\"background:blue\">" + editorAntecedent + "</span>&nbsp;");
  ...
  // 자동완성할 항목이 관용어구임
    ...
    if (workspaceStore.caseMasterData.format_code_path === "FORMAT/USPTO") {
      ...
      if (isFirstWordInSentence)
        tinymce
          .get(props.id)
          .insertContent(item.name[0]?.toUpperCase() + item.name.substring(1));
      else tinymce.get(props.id).insertContent(item.name);
    } else tinymce.get(props.id).insertContent(item.name);
  }
...
const checkAntecedentError = (item: ElementData, tagID: string) => {
  // console.log(
  //   props.claimEditor,
  //   claimCheckAntecedent,
  //   descriptionCheckAntecedent
  // );
  // 청구항 에디터인데, 청구항 상기 에러 체크 안한다고 되어 있으면 true 반환
  if (props.claimEditor && claimCheckAntecedent === "N") {
    return true;
  }

  // 청구항 에디터가 아닌데, 상세한 설명 상기 에러 체크 안한다고 되어 있으면 true 반환
  if (!props.claimEditor && descriptionCheckAntecedent === "N") {
    return true;
  }
  ...
  // 자동완성 키워드 앞의 text에 선행사 후보가 사용되었는지 확인해서, 사용되었으면 해당 선행사로 체크 진행
  editorAntecedent = "";
  for (let i = 0; i < editorAntecedentArray.length; i++) {
    const tempEditorAntecedent = editorAntecedentArray[i].trim();

    // 비정상 데이터면 pass
    if (!tempEditorAntecedent?.length) continue;

    // 자동완성 키워드 전의 텍스트의 길이가 선행사의 길이보다 길고, 마지막 단어가 체크용 선행사인 경우 체크 진행 (영어인 경우에는 소문자화해서 비교)
    if (
      checkTarget.length >= tempEditorAntecedent.length &&
      checkTarget.slice(0 - tempEditorAntecedent.length) ==
        tempEditorAntecedent.toLowerCase()
    ) {
      editorAntecedent = tempEditorAntecedent;
      break;
    }
  }
  ...
  if (editorAntecedent !== "") {
    ...
    tinymce.get(props.id).insertContent('<span id="' + tagID + '"></span>');
    let targetElement = tinymce.get(props.id).getDoc().getElementById(tagID);
    let contentHtml = tinymce.get(props.id).getContent();
    // 임시 태그 앞의 html 추출
...
const checkFirstWordInSentence = (range: Range) => {
  // 대문자 체크 안하는 경우에는 항상 false 리턴
  if (props.doNotUseAutoCapitalize) return false;
      ...
      if (
        ...
        if (josaAll.includes(checkTarget)) {
          ...
          if (checkTarget != checkResult) {
            ...
            // 정상 조사와 트리거 문자 추가
            tinymce.get(props.id).insertContent(checkResult + trigger);
          } else return;
        } else {
